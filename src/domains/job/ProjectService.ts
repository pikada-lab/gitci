...
export class ProjectService {
  ...
  constructor(private util: UtilitiesService, private gitService: GitService) {}

  async add(item: ProjectModel) {
    try {
      const project = new Project(item.id, item.git, item.name);

      item.pipelines.forEach((pipeline) => {
        const strategy = ConditionParser(pipeline.strategy);
        const steps = pipeline.steps.map(
          (step) => new Step(step.id, step.name, step.scripts)
        );
        project.addPipeline(
          new Pipeline(
            pipeline.id,
  ...
  private async init(project: Project) {
    ...
    project.events.addListener("task", (task) =>
