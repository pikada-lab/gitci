...
export class GitService implements IGitService {
  ...
  async getCommitBranch(repoPath: string, commit: Commit) {
    let branch = await this.gitCommand(
      repoPath,
      "branch",
      "--contains",
      commit.hash
  ...
  async log(repoPath: string): Promise<Commit[]> {
    let commitsObject = await streamToPromise(
      gitCommits(join(repoPath, ".git"), { limit: 1000 })
    );
    const commits = commitsObject.map((r) =>
      new Commit().setRef(r)
    ) as Commit[];
  ...
  async check(): Promise<boolean> {
    return new Promise((resolve, reject) => {
      ...
      git.stdout.on("data", (chunk: Buffer) => {
        const version = /(\d+)\.(\d+)\.\d+/i.exec(
          chunk.toString("utf8").substr(12, 7)
        );
        if (!version || !version[1] || !version[2]) {
          return reject("version problem");
        }
        if (
          +version[1] == this.versionMajor &&
          +version[2] >= this.versionMinor
        ) {
