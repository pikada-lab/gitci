...
export class NodeCI {
  ...
  async open(file: string) {
    ...
    this.projectService.events.addListener("task", (task: Task) =>
      this.addTask(task)
    );
  ...
  async save(file: string) {
    const projectStorage = new FileStorage<ProjectModel>(file);
    projectStorage.save(
      this.projectService.getProjects().map((r) => r.getModel())
    );
  }
  ...
  async addPipeline(
    ...
    project.addPipeline(
      new Pipeline(this.util.IDGen(), name, environment, conditions, [])
    );
