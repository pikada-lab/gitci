...
describe("Watcher.", () => {
  ...
  describe("getLastPullingDate()", () => {
    ...
    it("shoul be between 100ms", async () => {
      ...
      expect(watcher.getLastPullingDate().getTime()).toBeGreaterThan(
        Date.now() - 100
      );
    });
  });
  describe("getLastWatchStartingDate()", () => {
    ...
    it("shoul be between 100ms", async () => {
      ...
      await timeout(10);
      watcher.stop();
      expect(watcher.getLastWatchStartingDate().getTime()).toBeLessThan(
        Date.now()
      );
      expect(watcher.getLastWatchStartingDate().getTime()).toBeGreaterThan(
        Date.now() - 100
      );
    });
  });
